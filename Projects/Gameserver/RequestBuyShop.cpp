...
bool CUser::RequestBuyShop(PacketHeader *Header)
{
	...
	if (mobId < 1000)
	{
		...
		if (pMob[clientId].Mobs.Player.Gold < price)
		{
		...
		if (slotId == -1)
		{
			SendClientMessage(clientId, "Não ha espaço no inventario");

			return true;
		}

		pMob[clientId].Mobs.Player.Gold -= static_cast<int>(price);
	...
	if (distance > VIEWGRIDX / 2)
	{
		...
		Log(clientId, LOG_INGAME, "Uso do NPC %s fora do range para tentar comprar %ux %uy %ux %uy. Distância: %d", spw->Name, pMob[mobId].Target.X, pMob[mobId].Target.Y, pMob[clientId].Target.X, pMob[clientId].Target.Y, distance);
		return false;
	}

	if(spw->bStatus.Merchant.Merchant != 1)
	{
		Log(SERVER_SIDE, LOG_INGAME, "%s - Tentativa de comprar item de mob! Mob: %s. Posição: %dx %dy.", User.Username, spw->Name, pMob[mobId].Target.X, pMob[mobId].Target.Y);
	...
	STRUCT_ITEMLIST *itemList = &ItemList[item->Index];
	...
	if(impost >= 2000000LL)
	{
		...
		Log(SERVER_SIDE, LOG_INGAME, "%s - Comprou o item %s na loja de %s. Preço: %lld. Imposto: %lld. Gold: %d.", User.Username, itemList->Name, spw->Name, itemPrice, impost, pMob[clientId].Mobs.Player.Gold);
	}

	// Arrecada o imposto da cidade
	g_pCityZone[cityZone].impost += impost;

	Log(clientId, LOG_INGAME, "Comprou o item %s %s na loja de %s. Preço total: %lld. Imposto: %lld. Gold: %d anterior. Gold após a compra: %d", itemList->Name, item->toString().c_str(), spw->Name, itemPrice, impost, pMob[clientId].Mobs.Player.Gold, mySpawn->Gold - static_cast<int>(itemPrice));
