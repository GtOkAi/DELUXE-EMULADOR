...
bool CUser::HandleAdminCommand(p334* p)
{
	...
	else if (sscanf_s(p->eValue, "+set ban %s %d %d %d %d", &nickName, 16, &x2, &x, &y, &slot))
	{
	...
	else if (sscanf_s(p->eValue, "+set hp %d %[^\n] ", &x, nickName, 16))
	{
	...
	else if (sscanf_s(p->eValue, "+set newrandom %d", &x))
	{
	...
	else if (!strncmp(p->eValue, "+check", 6))
	{
	...
	else if (!strncmp(p->eValue, "+checkdoor", 11))
	{
	...
	else if (!strncmp(p->eValue, "+snoop", 6))
	{
...
#pragma endregion
	...
	/*else if(!strncmp(p->eValue, "+item", 5))
	{
		...
		SendClientMessage(clientId, "SET ITEM CARRY");
	...
	else if (!strncmp(p->eValue, "+learn", 6))
	{
