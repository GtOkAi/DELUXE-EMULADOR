...
int TMScene::OnPacketEvent(unsigned int dwCode, char* pSBuffer)
{
	...
	auto pStd = reinterpret_cast<PacketHeader*>(pSBuffer);
	...
	if (pStd->PacketId == MSG_Encode_Opcode)
	{
	...
	if (pStd->PacketId == 0xFBC)
	{
		// need to decompile here... and other encode packets
		return 1;
	}

	if (pStd->PacketId == 0x13BD)
	{
		// need to decompile here... and other encode packets
		return 1;
	}

	if (pStd->PacketId == 0x7BE)
	{
		// need to decompile here... and other encode packets
		return 1;
	}
	if (!pStd->ClientId && (pStd->PacketId == MSG_MessagePanel_Opcode || pStd->PacketId == 0x102 || pStd->PacketId == 0x104 || pStd->PacketId == 0x105 || pStd->PacketId == 0x106))
	{
		char szStr[128] = { 0 };
		if (pStd->PacketId != MSG_MessagePanel_Opcode)
		{
	...
	if (pStd->PacketId == MSG_MessageShout_Opcode)
	{
		...
		pShoutMessage->eCommand[111] = 0;
		pShoutMessage->eCommand[110] = 0;
		...
		if (!pFScene->m_pChatGeneral || pFScene->GetSceneType() != ESCENE_TYPE::ESCENE_FIELD || pFScene->m_pCHP->m_bSelectEnable)
		{
			...
			int nLen = strlen(pShoutMessage->eCommand);

			SListBoxItem* ipNewItem = new SListBoxItem(pShoutMessage->eCommand,
				dwColor,
				0.0f,
				0.0f,
...
void TMScene::LogMsgCriticalError(int Type, int ID, int nMesh, int X, int Y)
{
	...
	stLog.Header.ClientId = m_pMyHuman->m_dwID;
	stLog.Header.PacketId = MSG_MessageLog_Opcode;
