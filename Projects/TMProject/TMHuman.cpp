...
int TMHuman::OnPacketEvent(unsigned int dwCode, char* buf)
{
    ...
    PacketHeader* pStandard = (PacketHeader*)buf;

    if (pStandard->ClientId != m_dwID)
        return 0;
    ...
    case 0x36A:
        ...
        break;
    case 0x165:
        return OnPacketRemoveMob((PacketHeader*)buf);
        break;
    case 0x182:
        return OnPacketSendItem((PacketHeader*)buf);
        break;
    case MSG_UpdateEquip_Opcode:
        return OnPacketUpdateEquip((PacketHeader*)buf);
        break;
    case 0x3B9:
        return OnPacketUpdateAffect((PacketHeader*)buf);
        break;
    case 0x336:
    ...
    case 0x18A:
        ...
        break;
    case 0x333:
        return OnPacketMessageChat((PacketHeader*)buf);
        break;
    case 0x105:
        return OnPacketMessageChat_Index((PacketHeader*)buf);
        break;
    case 0x106:
    ...
    case 0x383:
        ...
        break;
    case 0x384:
        return OnPacketQuitTrade((PacketHeader*)buf);
        break;
    case 0x185:
        return OnPacketCarry(reinterpret_cast<MSG_Carry*>(buf));
        break;
    case 0x386:
        return OnPacketCNFCheck((PacketHeader*)buf);
        break;
    case 0x193:
    ...
    case 0x39F:
        ...
        break;
    case 0x3AD:
        return OnPacketVisualEffect((PacketHeader*)buf);
        break;
    default:
        break;    
    }
...
int TMHuman::OnPacketIllusion(PacketHeader* pStd)
{
    ...
    if (pScene->m_pMyHuman == this)
    {
        ...
        pScene->Warp();
        if (pStd->PacketId == MSG_Action2_Opcode)
...
int TMHuman::OnPacketFireWork(MSG_Motion* pStd)
{
	...
	if (pStd->Motion < 256)
	{
		if (g_pObjectManager->m_dwCharID == pStd->Header.ClientId)
...
int TMHuman::OnPacketPremiumFireWork(PacketHeader* pStd)
{
	return 0;
}

int TMHuman::OnPacketRemoveMob(PacketHeader* pStd)
{
...
int TMHuman::OnPacketSendItem(PacketHeader* pStd)
{
    ...
    { 
        if (pSendItem->invType == 0)
        { 
            if (pSendItem->invSlot == 6 && BASE_GetItemAbility(&g_pObjectManager->m_stMobData.Equip[6], EF_WTYPE) == 41)
            { 
                m_stLookInfo.RightMesh = 0;
                m_stLookInfo.RightSkin = 0;
            }

            memcpy(&pMobData->Equip[pSendItem->invSlot], &pSendItem->itemData, sizeof(STRUCT_ITEM));

            if (pSendItem->invSlot)
            { 
                memcpy(&g_pObjectManager->m_stSelCharData.Equip[g_pObjectManager->m_cCharacterSlot][pSendItem->invSlot], &pSendItem->itemData, sizeof(STRUCT_ITEM));
            ...
            if (pSendItem->invSlot > 0 && pSendItem->invSlot < 16)
            { 
                if (pGridEquip[pSendItem->invSlot] != nullptr)
                { 
                    SGridControlItem* pItem = pGridEquip[pSendItem->invSlot]->PickupItem(0, 0);
                ...
                if (pSendItem->itemData.sIndex > 0)
                { 
                    auto pstItem = new STRUCT_ITEM();

                    if (pstItem)
                    { 
                        memcpy(pstItem, &pMobData->Equip[pSendItem->invSlot], sizeof(STRUCT_ITEM));

                        auto pItem = new SGridControlItem(0, pstItem, 0.0f, 0.0f);

                        if (pItem)
                        { 
                            if (pGridEquip[pSendItem->invSlot])
                            { 
                                pGridEquip[pSendItem->invSlot]->Empty();
                                pGridEquip[pSendItem->invSlot]->AddItem(pItem, 0, 0);
                            }
                        }
                    }
                }
        ...
        else if (pSendItem->invType == 1)
        { 
            memcpy(&pMobData->Carry[pSendItem->invSlot], &pSendItem->itemData, sizeof(STRUCT_ITEM));
            memcpy(g_pObjectManager->m_stMobData.Carry, pMobData->Carry, sizeof(g_pObjectManager->m_stMobData.Carry));

            int Page = pSendItem->invSlot / 15;
            int CellIndexX = pSendItem->invSlot % 15 % 5;
            int CellIndexY = pSendItem->invSlot % 15 / 5;
            ...
            { 
                ...
                { 
                    ...
                    if (pstItem)
                    { 
                        memcpy(pstItem, &pSendItem->itemData, sizeof(STRUCT_ITEM));
        ...
        else if (pSendItem->invType == 2)
        { 
            memcpy(&g_pObjectManager->m_stItemCargo[pSendItem->invSlot], &pSendItem->itemData, sizeof(STRUCT_ITEM));

            int CellIndexX = pSendItem->invSlot % 40 % 5;
            int CellIndexY = pSendItem->invSlot % 40 / 5;

            auto pGrid = pFScene->m_pCargoGridList[pSendItem->invSlot / 40];
...
int TMHuman::OnPacketUpdateAffect(PacketHeader* pStd)
{
    ...
    if (pFScene && g_pCurrentScene->m_pMyHuman == this)
    {
        ...
        if (!pFScene->m_nYear && !pFScene->m_nDays || pFScene->m_dwEventTime && dwServerTime > (pFScene->m_dwEventTime + 3600000))
        {
            ...
            sprintf_s(stWhisper.eCommand, "day");
...
int TMHuman::OnPacketSetHpDam(PacketHeader* pStd)
{
...
int TMHuman::OnPacketMessageChat_Index(PacketHeader* pStd)
{
	return 0;
}

int TMHuman::OnPacketMessageChat_Param(PacketHeader* pStd)
{
	return 0;
}
...
int TMHuman::OnPacketMessageWhisper(MSG_MessageWhisper* pMsg)
{
    ...
    if (pMsg->eValue[0] == '-' && pMsg->Color == 3)
    {
        ...
        sprintf_s(szMsg, "[%s]> %s", pMsg->eCommand, &pMsg->eValue[nIndex]);
    }
    else if (pMsg->Color == 7)
    {
        dwColor = 0xFFBBBBBB;
        sprintf_s(szMsg, "[%s]> %s", pMsg->eCommand, pMsg->eValue);
    }
    ...
    else if (pMsg->eValue[0] != '!')
    {
        ...
        sprintf_s(szMsg, "[%s] : %s> %s", pMsg->eCommand, m_szName, pMsg->eValue);
        ...
        sprintf_s(szMsg, g_pMessageStringTable[226], pMsg->eCommand);
        ...
        sprintf_s(szMsg, "%s", &pMsg->eValue[1]);
    ...
    if (strlen(pMsg->eCommand) + strlen(szMsg) <= 55)
    {
        if (pChatList)
...
int TMHuman::OnPacketQuitTrade(PacketHeader* pStd)
{
    if (g_pCurrentScene->m_pMyHuman == this)
    {
        g_pObjectManager->m_stTrade.OpponentID = 0;
...
int TMHuman::OnPacketCNFCheck(PacketHeader* pStd)
{
...
int TMHuman::OnPacketVisualEffect(PacketHeader* pStd)
{
...
void TMHuman::MoveAttack(TMHuman* pTarget)
{
    ...
    if (nDistance <= nMobAttackRange && nTX == (int)pTarget->m_vecPosition.x && nTY == (int)pTarget->m_vecPosition.y)
    {
        ...
        stAttack.Header.ClientId = m_dwID;
        ...
        if (pHumanTarget)
        {
            ...
            pScene->OnPacketEvent(stAttack.Header.PacketId, (char*)&stAttackLocal);
...
void TMHuman::MoveGet(TMItem* pTarget)
{
    ...
    stGetItem.Header.ClientId = m_dwID;
    stGetItem.Header.PacketId = MSG_GetItem_Opcode;
                        ...
                        if (bStop && (m_LastSendTargetPos.x != vecTarget.x || m_LastSendTargetPos.y != vecTarget.y))
                        {
                                ...
                                && g_pCurrentScene->m_eSceneType != ESCENE_TYPE::ESCENE_DEMO)
                            {
                                ...
                                if (bStop != 2)
                                {
                                    ...
                                    g_bLastStop = dst.Header.PacketId;
                                }
...
int TMHuman::MAutoAttack(TMHuman* pTarget, int mode)
{
        ...
        m_LastSendTargetPos.x == (int)m_vecPosition.x && m_LastSendTargetPos.y == (int)m_vecPosition.y)
    {
        ...
        MSG_Attack stAttackLocal{};
        memcpy((char*)&stAttackLocal, (char*)&stAttack, nSize);
        stAttackLocal.Header.ClientId = pScene->m_dwID;
        stAttackLocal.FlagLocal = 1;
