...
int TMFieldScene::InitializeScene()
{
	...
	g_pObjectManager->m_stCombineItem.Header.ClientId = g_pObjectManager->m_dwCharID;
	g_pObjectManager->m_stCombineItem.Header.PacketId = 0x3A6;
	g_pObjectManager->m_stCombineItem4.Header.ClientId = g_pObjectManager->m_dwCharID;
	g_pObjectManager->m_stCombineItem4.Header.PacketId = 0x3C0;

	for (int i = 0; i < 15; ++i)
		g_pObjectManager->m_stTrade.CarryPos[i] = -1;
	for (int i = 0; i < 8; ++i)
	{
		g_pObjectManager->m_stCombineItem.slot[i] = -1;
		g_pObjectManager->m_stCombineItem4.slot[i] = -1;
	}
	...
	if (!strcmp(g_TempName, g_pObjectManager->m_stMobData.MobName))
	{
		if (strcmp(g_TempNick, " "))
		{
			...
			sprintf(stWhisper.eCommand, "tab");
			sprintf(stWhisper.eValue, g_TempNick);
...
int TMFieldScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	if (idwControlID == TMB_GAMBLE_START)
	{
		...
		if (pMobData->Coin - m_nBet >= 0)
		{
			...
			stParm2.Header.ClientId = g_pObjectManager->m_dwCharID;
			stParm2.Header.PacketId = MSG_DoJackpotBet_Opcode;
			stParm2.Parm1 = 1;
	...
	if (idwControlID == E_CHAT && !idwEvent)
	{
		...
		case '/':
		{
			...
			sprintf(stMsgWhisper.eCommand, "%s", str1);
			...
			if (str[0])
			{
				sprintf(stMsgWhisper.eValue, "%s", &pChatText[strlen(str1) + 2]);
			}

			BASE_TransCurse(stMsgWhisper.eValue);
			...
			{
				sprintf(stMsgWhisper.eCommand, "%s", "spk");
			...
			else if (!strcmp(str1, g_pMessageStringTable[386]))
			{
				...
				if (!CheckGuildName(stMsgWhisper.eValue, 0))
				{
					m_pMessagePanel->SetMessage(g_pMessageStringTable[370], 2000);
					m_pMessagePanel->SetVisible(1, 1);
					return 1;
				}
				sprintf(stMsgWhisper.eCommand, "%s", "create");
				SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
			}
			else if (!strcmp(str1, g_pMessageStringTable[387]))
			{
				...
				sprintf(stMsgWhisper.eCommand, "%s", "handover");
				SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
			}
			else if (!strcmp(str1, g_pMessageStringTable[391]))
			{
				sprintf(stMsgWhisper.eCommand, "%s", "getout");
				SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
			}
			else if (!strcmp(str1, g_pMessageStringTable[390]))
			{
				sprintf(stMsgWhisper.eCommand, "%s", "war");
				SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
			}
			...
			{
				sprintf(stMsgWhisper.eCommand, "%s", "item_lock");
				SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
			}
			else if (!strcmp(str1, g_pMessageStringTable[497]))
			{
				sprintf(stMsgWhisper.eCommand, "%s", "item_unlock");
				SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
			}
			else if (!strcmp(str1, "tab"))
			{
				...
				if (!strcmp(m_pMyHuman->m_szNickName, stMsgWhisper.eValue))
					return 1;

				sprintf(g_TempNick, "%s", stMsgWhisper.eValue);
			...
			if (!strcmp(str1, "r") || !(strcmp(str1, "re")))
			{
				...
				sprintf(istrText, "[%s] [%s]> %s", g_pObjectManager->m_stMobData.MobName, str1, stMsgWhisper.eCommand);
			}
			else
			{
				sprintf(istrText, "[%s] : %s> %s", g_pObjectManager->m_stMobData.MobName, str1, stMsgWhisper.eCommand);
			...
			else
			{
				char dest[128]{};
				char dest2[128]{};
				if (IsClearString(stMsgWhisper.eValue, maxLen - 1))
				{
					strncpy(dest, stMsgWhisper.eValue, maxLen);
					sprintf(dest2, "%s", &stMsgWhisper.eValue[maxLen]);
				}
				else
				{
					strncpy(dest, stMsgWhisper.eValue, maxLen - 1);
					sprintf(dest2, "%s", &stMsgWhisper.eCommand[maxLen + 15]);
				}
	...
	if (idwControlID == B_IG_OK)
	{
		...
		if (strcmp(inputText, "all"))
		{
			...
			case 2:
			{
				...
				stWithdraw.Header.PacketId = MSG_Withdraw_Opcode;
				stWithdraw.Header.ClientId = m_pMyHuman->m_dwID;
			...
			case 0:
			{
				...
				stDeposit.Header.PacketId = MSG_Deposit_Opcode;
				stDeposit.Header.ClientId = m_pMyHuman->m_dwID;
			...
			case 1:
			{
				...
				g_pObjectManager->m_stTrade.Header.PacketId = MSG_Trade_Opcode;
			...
			case 7:
			{
				...
				stPacket.Header.PacketId = 977;
				stPacket.Header.ClientId = m_pMyHuman->m_dwID;
			...
			case 8:
			{
				...
				sprintf_s(stMessageWhisper.eCommand, "subcreate");
				sprintf_s(stMessageWhisper.eValue, "%s", str2);
				...
				if (!CheckGuildName(stMessageWhisper.eValue, 1))
				{
					m_pMessagePanel->SetMessage(g_pMessageStringTable[370], 2000);
					m_pMessagePanel->SetVisible(1, 1);
					return 1;
				...
				{
					sprintf(stMessageWhisper.eValue, "%s", &text[strlen(str2) + 2]);
				}

				BASE_TransCurse(stMessageWhisper.eValue);
			...
			case 9:
			{
				...
				stPacket.Header.PacketId = 0xED7;
				stPacket.Header.ClientId = m_pMyHuman->m_dwID;
				stPacket.Parm = static_cast<int>(nInputValue);
				SendOneMessage((char*)&stPacket, sizeof(stPacket));
			}
			...
			case 10:
			{
				...
				stPacket.Header.PacketId = 0xED8;
				stPacket.Header.ClientId = m_pMyHuman->m_dwID;
				stPacket.Parm = static_cast<int>(nInputValue);
				SendOneMessage((char*)&stPacket, sizeof(stPacket));
			}
			...
			case 12:
			{
				...
				stPacket.Header.PacketId = MSG_SplitItem_Opcode;
				stPacket.Header.ClientId = m_pMyHuman->m_dwID;
				stPacket.Parm1 = pos;
	...
	if (idwControlID == B_SYS_QUIT)
	{
		...
		stParm.Header.ClientId = m_pMyHuman->m_dwID;
		stParm.Header.PacketId = MSG_SysQuit_Opcode;
	...
	if (idwControlID >= 897 && idwControlID <= 900)
	{
		m_pQuizBG->SetVisible(0);
		MSG_STANDARDPARM stParm{};
		stParm.Header.ClientId = g_pObjectManager->m_dwCharID;
		stParm.Header.PacketId = 0x2C7;
	...
	if (idwControlID == 12545)
	{
		if (m_bAirmove_ShowUI == 1)
			return 1;
		if (m_stPotalItem.Header.ClientId)
	...
	if (idwControlID == 12546)
	{
		...
		if (m_stPotalItem.Header.ClientId)
		{
			...
			if (pItem)
			{
				...
				memset(&m_stUseItem, 0, sizeof(m_stUseItem));
				m_stUseItem.Header.ClientId = g_pObjectManager->m_dwCharID;
				m_stUseItem.Header.PacketId = m_stPotalItem.Header.PacketId;
	...
	if (idwControlID == 641)
	{
		...
		if (!m_pPartyList->m_pItemList[0] || static_cast<SListBoxPartyItem*>(m_pPartyList->m_pItemList[0])->m_nState != 1)
		{
			...
			stReqParty.Header.PacketId = MSG_REQParty_Opcode;
			stReqParty.Header.ClientId = m_pMyHuman->m_dwID;
	...
	if (idwControlID == 643)
	{
		...
		{
			g_pObjectManager->m_stTrade.Header.PacketId = 0x383;
			g_pObjectManager->m_stTrade.Header.ClientId = m_pMyHuman->m_dwID;
	...
	if (idwControlID == 912)
	{
		...
		{
			MSG_STANDARDPARM2 stParm2{};
			stParm2.Header.PacketId = MSG_InviteGuild_Opcode;
			stParm2.Header.ClientId = m_pMyHuman->m_dwID;
	...
	if (idwControlID == 1617)
	{
		...
		sprintf(stMsgWhisper.eCommand, "_RPS_");
		sprintf(stMsgWhisper.eValue, "rock");
		SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
		m_pRPSGamePanel->SetVisible(0);
		return 0;
	}
	if (idwControlID == 1618)
	{
		...
		sprintf(stMsgWhisper.eCommand, "_RPS_");
		sprintf(stMsgWhisper.eValue, "paper");
		SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
		m_pRPSGamePanel->SetVisible(0);
		return 0;
	}
	if (idwControlID == 1619)
	{
		...
		sprintf(stMsgWhisper.eCommand, "_RPS_");
		sprintf(stMsgWhisper.eValue, "scissor");
		SendOneMessage((char*)&stMsgWhisper, sizeof(stMsgWhisper));
		m_pRPSGamePanel->SetVisible(0);
		return 0;
	}
	...
	if (idwControlID == 475138)
	{
		...
		if (pPartyItem && pPartyItem->m_nState == 1 && !pPartyList->m_bRButton)
		{
			...
			stCNFParty2.Header.ClientId = m_pMyHuman->m_dwID;
			stCNFParty2.Header.PacketId = MSG_CNFParty2_Opcode;
		...
		else if (pPartyItem && pPartyList->m_bRButton == 1)
		{
					...
					|| skillId == 13))
			{
				...
				MSG_Attack stAttack{};
				stAttack.Header.PacketId = MSG_Attack_Multi_Opcode;
				...
				localAttack.Header.ClientId = m_dwID;
	...
	if (idwControlID == 667)
	{
		...
		m_stAutoTrade.Header.PacketId = 919;
		m_stAutoTrade.TargetID = m_pMyHuman->m_dwID;
...
int TMFieldScene::OnKeyDownEvent(unsigned int iKeyCode)
{
	...
	if (iKeyCode == VK_INSERT)
	{
		...
		sprintf_s(stWhisper.eCommand, "time");
	...
	if (iKeyCode == VK_F11)
	{
		...
		if (bFind == 1 && pItem)
		{
			if (BASE_GetItemAbility(pItem->m_pItem, 38) == 11)
			{
				...
				stDelayStart.Header.ClientId = m_pMyHuman->m_dwID;
				stDelayStart.Header.PacketId = 942;
				stDelayStart.Parm = 1;
				SendOneMessage((char*)&stDelayStart, sizeof(MSG_STANDARDPARM));

				m_stUseItem.Header.ClientId = g_pObjectManager->m_dwCharID;
				m_stUseItem.Header.PacketId = MSG_UseItem_Opcode;
		...
		if (g_pObjectManager->m_stMobData.CurrentScore.Hp > 0)
		{
			...
			if (nX > 0 && nY > 0 && nX < SWidth && nY < SHeight)
			{
				...
				else
				{
					...
					stMotion.Header.ClientId = g_pObjectManager->m_dwCharID;
					stMotion.Header.PacketId = MSG_Motion_Opcode;
...
int TMFieldScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	auto pStd = (PacketHeader*)buf;

	switch (pStd->PacketId)
	{
...
int TMFieldScene::FrameMove(unsigned int dwServerTime)
{
	...
	{
		PacketHeader stStandard;
		stStandard.ClientId = g_pObjectManager->m_dwCharID;
		stStandard.PacketId = 0x289;
	...
	if (!g_bCastleWar2)
	{
		...
		if (bInTown == 1)
		{
			...
			if (DS_SOUND_MANAGER::m_nMusicIndex != nMusicIndex)
			{
				...
				dst.Header.ClientId = g_pObjectManager->m_dwCharID;
				dst.Header.PacketId = 0x291;
	...
	else
	{
		if (bInTown == 1)
		{
			...
			if (m_nVillage != nVillage)
			{
				...
				stParam.Header.ClientId = g_pObjectManager->m_dwCharID;
				stParam.Header.PacketId = 0x291;
	...
	if (!m_pMyHuman->m_cLastMoveStop)
	{
		if (m_pMyHuman->m_fMaxSpeed >= 5.0f)
		{
			if (m_pMyHuman->m_fProgressRate >= 0.8f || m_pMyHuman->m_fProgressRate == 0.0f)
			{
					...
					dwServerTime - m_pMyHuman->m_dwOldMovePacketTime > 1000 && !m_pMyHuman->m_cDie)
				{
					...
					g_bLastStop = Msg.Header.PacketId;
		...
		else if (m_pMyHuman->m_fProgressRate >= 0.90f || m_pMyHuman->m_fProgressRate == 0.0f)
		{
				...
				m_pMyHuman->m_LastSendTargetPos.y != m_vecMyNext.y) && dwServerTime - m_pMyHuman->m_dwOldMovePacketTime > 1000 && !m_pMyHuman->m_cDie)
			{
				...
				g_bLastStop = stAction.Header.PacketId;
...
int TMFieldScene::SkillUse(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime, int bMoving, TMHuman* pTarget)
{
			...
			g_pSpell[(unsigned char)cSkillIndex].TargetType == 6))
	{
		...
		if ((int)stAttack.Dam[0].TargetID > 0 || cSkillIndex == 97 || cSkillIndex == 35 || cSkillIndex == 51)
		{
			...
			stAttackLocal.Header.ClientId = m_dwID;
			stAttackLocal.FlagLocal = 1;
			if (nSpecForce)
				stAttackLocal.DoubleCritical |= 8;

			OnPacketEvent(stAttack.Header.PacketId, (char*)&stAttackLocal);
	...
	stLocalAttack.Header.ClientId = m_dwID;
	stLocalAttack.FlagLocal = 1;

	if (nSpecForce)
		stLocalAttack.DoubleCritical |= 8u;

	OnPacketEvent(stAttack.Header.PacketId, (char*)&stLocalAttack);
	m_dwOldAttackTime = dwServerTime;
	m_dwSkillLastTime[(unsigned char)cSkillIndex] = dwServerTime;
...
int TMFieldScene::AutoSkillUse(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime, int bMoving, TMHuman* pTarget)
{
	...
	if (!TarType || TarType == 3 || TarType == 4 || TarType == 5 || TarType == 6)
	{
		...
		if (cSkillIndex == 27)
		{
			...
			MSG_Attack Msg{};
			Msg.Header.PacketId = MSG_Attack_Multi_Opcode;
			Msg.Header.ClientId = m_pMyHuman->m_dwID;
...
int TMFieldScene::MouseClick_NPC(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime)
{
	...
	if (pOver->m_TradeDesc[0])
	{
		...
		stQuest.Header.PacketId = 0x39A;
		stQuest.Header.ClientId = m_pMyHuman->m_dwID;
		...
		m_pGround->m_vecOffsetIndex.y != 16)
	{
		...
		if (pOver->m_dwID >= 1000 && (pOver->m_stScore.Reserved & 0xF) >= 6 && (pOver->m_stScore.Reserved & 0xF) <= 8)
		{
			...
			stPacket.Header.PacketId = 0x28E;
			stPacket.Header.ClientId = m_pMyHuman->m_dwID;
	...
	if (pOver && pOver->m_bMouseOver == 1)
	{
		...
		if (!IsTargetUser && !pOver->m_bParty && !pOver->m_cSummons || pOver->IsInPKZone())
		{
			...
			if (nDistance <= nMobAttackRange && nTX == (signed int)pOver->m_vecPosition.x && nTY == (signed int)pOver->m_vecPosition.y)
			{
				...
				{
					m_stMoveStop.Header.ClientId = m_pMyHuman->m_dwID;
					m_stMoveStop.Header.PacketId = 0x2CB;
...
void TMFieldScene::DropItem(unsigned int dwServerTime)
{
	...
	MSG_DropItem stDrop{};
	stDrop.Header.ClientId = m_pMyHuman->m_dwID;
	stDrop.Header.PacketId = MSG_DropItem_Opcode;
	stDrop.Rotate = 0;
...
int TMFieldScene::TimeDelay(unsigned int dwServerTime)
{
	...
	if (m_dwLastTown)
	{
		...
		{
			PacketHeader stRecall{};
			stRecall.ClientId = m_pMyHuman->m_dwID;
			stRecall.PacketId = MSG_Recall_Opcode;
	...
	if (m_dwLastResurrect)
	{
		...
		else
		{
			...
			if (m_dwLastRemain != dwRemain)
			{
				...
				OnPacketEvent(stAttack.Header.PacketId, (char*)&stAttack);
			}
		}
	}
	if (m_dwLastTeleport)
	{
		...
		else if (dwServerTime > m_dwLastTeleport + 5000 && m_cLastTeleport == 1)
		{
			if (m_nServerMove)
			{
				...
				stWhisper.Header.ClientId = g_pObjectManager->m_dwCharID;
				stWhisper.Header.PacketId = MSG_MessageWhisper_Opcode;
				sprintf(stWhisper.eCommand, "srv");
				sprintf(stWhisper.eValue, "%d", m_nServerMove);
				SendOneMessage((char*)&stWhisper, sizeof(stWhisper));
				m_nServerMove = 0;
			}
			else
	...
	if (m_dwLastRelo)
	{
		...
		else if (dwServerTime > m_dwLastRelo + 5000 && m_cLastRelo == 1)
		{
			...
			sprintf(stMsgImp.eCommand, "relo");
			sprintf(stMsgImp.eValue, "%s", m_szSummoner);
		...
		else if (dwServerTime > m_dwLastRelo + 5000 && m_cLastRelo == 2)
		{
			...
			stMsgImp.Header.PacketId = MSG_MessageWhisper_Opcode;
			sprintf(stMsgImp.eCommand, "relo");
			sprintf(stMsgImp.eValue, "%s", m_szSummoner2);
			SendOneMessage((char*)&stMsgImp, sizeof(stMsgImp));
			m_cLastRelo = 0;
		}
		else
...
void TMFieldScene::SetVisibleTrade(int bShow)
{
	...
	if (!bShow && g_pObjectManager->m_stTrade.OpponentID > 0)
	{
		...
		if (bSendQuit)
		{
			...
			stStandard.PacketId = MSG_CloseTrade_Opcode;
			stStandard.ClientId = m_pMyHuman->m_dwID;
			SendOneMessage((char*)&stStandard, sizeof(stStandard));
		}
	}
...
void TMFieldScene::ClearCombine()
{
	...
	memset(&g_pObjectManager->m_stCombineItem, 0, sizeof(g_pObjectManager->m_stCombineItem));
	g_pObjectManager->m_stCombineItem.Header.ClientId = m_pMyHuman->m_dwID;
	g_pObjectManager->m_stCombineItem.Header.PacketId = MSG_CombineItem_Opcode;
	for (int i = 0; i < 8; ++i)
		g_pObjectManager->m_stCombineItem.slot[i] = -1;
...
void TMFieldScene::ClearCombine4()
{
	...
	memset(&g_pObjectManager->m_stCombineItem4, 0, sizeof(g_pObjectManager->m_stCombineItem4));
	g_pObjectManager->m_stCombineItem4.Header.ClientId = m_pMyHuman->m_dwID;
	g_pObjectManager->m_stCombineItem4.Header.PacketId = MSG_CombineItemTiny_Opcode;
	for (int i = 0; i < 8; ++i)
		g_pObjectManager->m_stCombineItem4.slot[i] = -1;
...
void TMFieldScene::DoCombine()
{
	if (g_nCombineMode == 1)
	{
		for (int i = 0; i < 6; ++i)
		{
			if (!g_pObjectManager->m_stCombineItem.items[i].sIndex)
			{
	...
	for (int i = 0; i < 8; ++i)
	{
		if (g_pObjectManager->m_stCombineItem.slot[i] != -1)
		{
			bFind = true;
			break;
		}
...
{
	if (g_pObjectManager->m_stCombineItem4.items[0].sIndex && g_pObjectManager->m_stCombineItem4.items[1].sIndex)
	{
		bool bFind = false;
		for (int i = 0; i < 8; ++i)
		{
			if (g_pObjectManager->m_stCombineItem4.slot[i] != -1)
			{
				bFind = true;
				break;
			}
...
void TMFieldScene::FindAuto()
{
	...
	if (dwServerTime - LastSendTime > 250000)
	{
		PacketHeader stStandard{};
		stStandard.ClientId = g_pObjectManager->m_dwCharID;
		stStandard.PacketId = MSG_Ping_Opcode;
		SendOneMessage((char*)&stStandard, sizeof(stStandard));
	}
...
void TMFieldScene::SetPK()
{
	...
	MSG_STANDARDPARM stParam{};
	stParam.Header.ClientId = g_pObjectManager->m_dwCharID;
	stParam.Header.PacketId = MSG_SetPKMode_Opcode;
...
void TMFieldScene::SetVisibleAutoTrade(int bShow, int bCargo)
{
	...
	if (m_pAutoTrade)
	{
		...
		else
		{
			...
			for (int k = 0; k < 10; ++k)
			{
				m_stAutoTrade.Header.ClientId = m_pMyHuman->m_dwID;
...
int TMFieldScene::GetItemFromGround(unsigned int dwServerTime)
{
	...
	stUpdateItem.Header.ClientId = m_pMyHuman->m_dwID;
	stUpdateItem.ItemID = pOverItem->m_dwID;
	stUpdateItem.Header.PacketId = MSG_UpdateItem_Opcode;
...
char TMFieldScene::UseHPotion()
{
	...
	if (BASE_GetItemAbility(pItem->m_pItem, 38) == 1)
	{
		...
		stUseItem.Header.ClientId = g_pObjectManager->m_dwCharID;
		stUseItem.Header.PacketId = MSG_UseItem_Opcode;
...
void TMFieldScene::UseFireWork()
{
	...
	stUseItem.Header.ClientId = g_pObjectManager->m_dwCharID;
	stUseItem.Header.PacketId = MSG_UseItem2_Opcode;
	stUseItem.SourType = 1;
	stUseItem.SourPos = SourPos;
...
void TMFieldScene::VisibleInputCharName(SGridControlItem* pItem, int nCellX, int nCellY)
{
	...
	if (!m_dwUseItemTime || (dwServerTime - m_dwUseItemTime) >= 200)
	{
		...
		memset(&m_stCapsuleItem, 0, sizeof(m_stCapsuleItem));
		m_stCapsuleItem.Header.ClientId = g_pObjectManager->m_dwCharID;
		m_stCapsuleItem.Header.PacketId = 972;
...
int TMFieldScene::OnMsgBoxEvent(unsigned int idwControlID, unsigned int idwEvent, unsigned int dwServerTime)
{
	...
	case 601:
	{
		...
		MSG_Trade stTrade{};
		memcpy(&stTrade, &g_pObjectManager->m_stTrade, sizeof(stTrade));
		stTrade.Header.ClientId = m_pMyHuman->m_dwID;
		stTrade.Header.PacketId = MSG_Trade_Opcode;
		SendOneMessage((char*)&stTrade, sizeof(stTrade));
	...
	case 6196:
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (vecGrid.x > -1 && vecGrid.y > -1)
			{
				...
				stHellBuy.Header.ClientId = m_pMyHuman->m_dwID;
				stHellBuy.Header.PacketId = 701;
	...
	case 816:
	{
		...
		stGuildDep.Header.ClientId = m_pMyHuman->m_dwID;
		stGuildDep.Header.PacketId = 652;
	...
	case 817:
	{
		...
		stParam.Header.ClientId = m_pMyHuman->m_dwID;
		stParam.Header.PacketId = 3598;
	...
	case 13:
	{
		...
		stParam.Header.PacketId = MSG_Quest_Opcode;
		stParam.Header.ClientId = m_pMyHuman->m_dwID;
	...
	case 50001:
	{
		...
		stRemoveParty.Header.PacketId = 894;
		stRemoveParty.Header.ClientId = m_pMyHuman->m_dwID;
	...
	case 51:
	{
		...
		stQuest.Header.PacketId = MSG_Quest_Opcode;
		stQuest.Header.ClientId = m_pMyHuman->m_dwID;
		stQuest.Parm1 = m_pMessageBox->m_dwArg;
		stQuest.Parm2 = 0;
		SendOneMessage((char*)&stQuest, sizeof(stQuest));
	...
	case 740:
	{
		...
		stDeleteItem.Header.ClientId = m_pMyHuman->m_dwID;
		stDeleteItem.Header.PacketId = MSG_DeleteItem_Opcode;
	...
	case 890:
	{
		...
		if (pSellItem)
		{
			...
			stSell.Header.ClientId = m_pMyHuman->m_dwID;
			stSell.Header.PacketId = MSG_Sell_Opcode;
...
{	
	...
	if (!g_pObjectManager->GetHumanByID(pStd->Header.ClientId))
	{
		auto pPartyList = m_pPartyList;
		for (int i = 0; i < pPartyList->m_nNumItem; ++i)
		{
			auto pPartyItem = (SListBoxPartyItem*)pPartyList->m_pItemList[i];
			if (pPartyItem->m_dwCharID == pStd->Header.ClientId)
			{
				pStd->String[127] = 0;
				pStd->String[126] = 0;
...
int TMFieldScene::OnPacketMessageChat_Param(PacketHeader* pStd)
{
	return 0;
}
...
int TMFieldScene::OnPacketMessageWhisper(MSG_MessageWhisper* pMsg)
{
	...
	if (pMsg->eValue[0] == '-' && pMsg->Color == 3)
	{
		...
		sprintf(szMsg, "[%s]> %s", pMsg->eCommand, &pMsg->eValue[nIndex]);
	}
	else if (pMsg->Color == 7)
	{
		dwColor = 0xFFBBBBBB;
		nIndex = 0;
		sprintf(szMsg, "[%s]> %s", pMsg->eCommand, pMsg->eValue);
	}
	else if (pMsg->eValue[0] == '=')
	{
	...
	else if (pMsg->eValue[0] != '!')
	{
		...
		sprintf(szMsg, g_pMessageStringTable[226], pMsg->eCommand);
		char szTime[128]{};
	...
	if (strlen(pMsg->eCommand) + strlen(pMsg->eValue) <= 43)
	{
		sprintf(szMsg, "[%s]> %s", pMsg->eCommand, &pMsg->eValue[nIndex]);
	...
	else
	{
		...
		if (IsClearString(pMsg->eValue, 42))
		{
			strncpy(szMsg3, pMsg->eValue, 43);
			sprintf(szMsg2, "%s", &pMsg->eValue[43]);
		}
		else
		{
			strncpy(szMsg3, pMsg->eValue, 42);
			sprintf(szMsg2, "%s", &pMsg->eValue[42]);
		}

		sprintf(szMsg, "[%s]> %s", pMsg->eCommand, &szMsg3[nIndex]);

		if(pChatList)
			pChatList->AddItem(new SListBoxItem(szMsg, dwColor, 0.0f, 0.0f, 280.0f, 16.0f, 0, 0x77777777, 1, 0));

		if (strlen(pMsg->eValue) > 43 && pChatList)
...
int TMFieldScene::OnPacketCancelSummon(PacketHeader* pStd)
{
	m_pHelpSummon->SetVisible(0);
	return 1;
}

int TMFieldScene::OnPacketAction(PacketHeader* pStd)
{
	if (g_pObjectManager->GetHumanByID(pStd->ClientId))
		return 0;
	
	MSG_REQMobByID stReqMobById{};

	stReqMobById.Header.ClientId = g_pObjectManager->m_dwCharID;
	stReqMobById.Header.PacketId = MSG_REQMobByID_Opcode;
	stReqMobById.MobID = pStd->ClientId;
...
int TMFieldScene::OnPacketCreateMob(PacketHeader* pStd)
{
	...
	if (!pNode)
	{
		...
		if (usGuild)
		{
			...
			if (pStd->PacketId != MSG_CreateMobTrade_Opcode && !pHuman->m_pAutoTradeDesc->IsVisible())
...
int TMFieldScene::OnPacketCNFCharacterLogout(PacketHeader* pStd)
{
	if (pStd->ClientId == g_pObjectManager->m_dwCharID)
	{
		g_pDevice->m_nWidthShift = 0;
...
{
	if (pStd->Header.ClientId != g_pObjectManager->m_dwCharID || g_pSocketManager->Sock)
	{
		if (pStd->Header.ClientId == g_pObjectManager->m_dwCharID && g_pSocketManager->Sock)
		{
	...
	{
		p20D stAccountLogin{};
		stAccountLogin.Header.ClientId = 0;
		stAccountLogin.Header.PacketId = MSG_AccountLogin_Opcode;
		stAccountLogin.CliVer = 759;
		stAccountLogin.Unknow_84 = 1;
		...
		strncpy(stAccountLogin.Login, pStd->AccountName, sizeof(pStd->AccountName));
		strncpy(stAccountLogin.Unknow, pStd->TID, sizeof(pStd->TID));
		sprintf(stAccountLogin.Password, "");
...
int TMFieldScene::OnPacketSwapItem(PacketHeader* pStd)
{
	...
	if (!pSwapItem->SrcType)
	{
		...
		pSrcGrid = pGridSrc[pSwapItem->SrcSlot];
		pSrcItem = pSrcGrid->PickupItem(0, 0);

		memset(&g_pObjectManager->m_stMobData.Equip[pSwapItem->SrcSlot], 0, sizeof(STRUCT_ITEM));
	}
	else if (pSwapItem->SrcType == 1)
	{
		pSrcGrid = m_pGridInvList[pSwapItem->SrcSlot / 15];
		pSrcItem = pSrcGrid->PickupAtItem(pSwapItem->SrcSlot % 15 % 5, pSwapItem->SrcSlot % 15 / 5);
		memset(&g_pObjectManager->m_stMobData.Carry[pSwapItem->SrcSlot], 0, sizeof(STRUCT_ITEM));
	}
	else if (pSwapItem->SrcType == 2)
	{
		pSrcGrid = m_pCargoGridList[pSwapItem->SrcSlot / 40];
		pSrcItem = pSrcGrid->PickupAtItem(pSwapItem->SrcSlot % 40 % 5, pSwapItem->SrcSlot % 40 / 5);
		memset(&g_pObjectManager->m_stItemCargo[pSwapItem->SrcSlot], 0, sizeof(STRUCT_ITEM));
	}

	SGridControl* pGridDest[16]{};
	if (!pSwapItem->DstType)
	{
		...
		pDestGrid = pGridDest[pSwapItem->DstSlot];
		pDestItem = pDestGrid->PickupItem(0, 0);

		memset(&g_pObjectManager->m_stMobData.Equip[pSwapItem->DstSlot], 0, sizeof(STRUCT_ITEM));
	}
	else if (pSwapItem->DstType == 1)
	{
		pDestGrid = m_pGridInvList[pSwapItem->DstSlot / 15];
		pDestItem = pDestGrid->PickupAtItem(pSwapItem->DstSlot % 15 % 5, pSwapItem->DstSlot % 15 / 5);
		memset(&g_pObjectManager->m_stMobData.Carry[pSwapItem->DstSlot], 0, sizeof(STRUCT_ITEM));
	}
	else if (pSwapItem->DstType == 2)
	{
		pDestGrid = m_pCargoGridList[pSwapItem->DstSlot / 40];
		pDestItem = pDestGrid->PickupAtItem(pSwapItem->DstSlot % 40 % 5,
			 pSwapItem->DstSlot % 40 / 5);
		memset(&g_pObjectManager->m_stItemCargo[pSwapItem->DstSlot], 0, sizeof(STRUCT_ITEM));
	}
	...
	if (pDestItem)
	{
		...
		if (!pSwapItem->SrcType)
		{
			if (pDestItem->m_pItem->sIndex > 40)
			{
				...
				if (pDestItem)
					memcpy(&g_pObjectManager->m_stMobData.Equip[pSwapItem->SrcSlot], pDestItem->m_pItem, sizeof(STRUCT_ITEM));				
				else
					memset(&g_pObjectManager->m_stMobData.Equip[pSwapItem->SrcSlot], 0, sizeof(STRUCT_ITEM));
			}
			else
		...
		else if (pSwapItem->SrcType == 1)
		{
			if (pDestItem->m_pItem->sIndex > 40)
			{				
				pSrcGrid->AddItem(pDestItem, pSwapItem->SrcSlot % 15 % 5, pSwapItem->SrcSlot % 15 / 5);
				if (pDestItem)
					memcpy(&g_pObjectManager->m_stMobData.Carry[pSwapItem->SrcSlot], pDestItem->m_pItem, sizeof(STRUCT_ITEM));
				else
					memset(&g_pObjectManager->m_stMobData.Carry[pSwapItem->SrcSlot], 0, sizeof(STRUCT_ITEM));
			}
			else
		...
		else if (pSwapItem->SrcType == 2)
		{
			if (pDestItem->m_pItem->sIndex > 40)
			{				
				pSrcGrid->AddItem(pDestItem, pSwapItem->SrcSlot % 40 % 5, pSwapItem->SrcSlot % 40 / 5);
				if (pDestItem)
					memcpy(&g_pObjectManager->m_stItemCargo[pSwapItem->SrcSlot], pDestItem->m_pItem, sizeof(STRUCT_ITEM));
				else
					memset(&g_pObjectManager->m_stItemCargo[pSwapItem->SrcSlot], 0, sizeof(STRUCT_ITEM));
			}
			else
	...
	if (pSrcItem)
	{
		if (!pSwapItem->DstType)
		{
			if (pSrcItem->m_pItem->sIndex > 40)
			{
				...
				if (pSrcItem)
					memcpy(&g_pObjectManager->m_stMobData.Equip[pSwapItem->DstSlot], pSrcItem->m_pItem, sizeof(STRUCT_ITEM));
				else
					memset(&g_pObjectManager->m_stMobData.Equip[pSwapItem->DstSlot], 0, sizeof(STRUCT_ITEM));
			}
			else
		...
		else if (pSwapItem->DstType == 1)
		{
			if (pSrcItem->m_pItem->sIndex > 40)
			{
				pDestGrid->AddItem(pSrcItem, pSwapItem->DstSlot % 15 % 5, pSwapItem->DstSlot % 15 / 5);
				if (pSrcItem)
					memcpy(&g_pObjectManager->m_stMobData.Carry[pSwapItem->DstSlot], pSrcItem->m_pItem, sizeof(STRUCT_ITEM));
				else
					memset(&g_pObjectManager->m_stMobData.Carry[pSwapItem->DstSlot], 0, sizeof(STRUCT_ITEM));
			}
			else
		...
		else if (pSwapItem->DstType == 2)
		{
			if (pSrcItem->m_pItem->sIndex > 40)
			{
				pDestGrid->AddItem(pSrcItem, pSwapItem->DstSlot % 40 % 5, pSwapItem->DstSlot % 40 / 5);
				if (pSrcItem)
					memcpy(&g_pObjectManager->m_stItemCargo[pSwapItem->DstSlot], pSrcItem->m_pItem, sizeof(STRUCT_ITEM));
				else
					memset(&g_pObjectManager->m_stItemCargo[pSwapItem->DstSlot], 0, sizeof(STRUCT_ITEM));
			}
			else
	...
	if (pSwapItem->SrcSlot == 15 && pSwapItem->DstSlot != 15 && m_pMyHuman)
...
int TMFieldScene::OnPacketBuy(PacketHeader* pStd)
{
	// Just that
	return 1;
}

int TMFieldScene::OnPacketSell(PacketHeader* pStd)
{
...
int TMFieldScene::OnPacketDeposit(PacketHeader* pStd)
{
...
int TMFieldScene::OnPacketWithdraw(PacketHeader* pStd)
{
...
int TMFieldScene::OnPacketReqChallange(PacketHeader* pStd)
{
	if (!m_pMessageBox->IsVisible())
	{
...
int TMFieldScene::OnPacketCloseShop(PacketHeader* pStd)
{
	SetVisibleShop(0);
	return 1;
}
...
int TMFieldScene::OnPacketClearMenu(PacketHeader* pStd)
{
	SetVisibleCargo(0);
	return 1;
}

int TMFieldScene::OnPacketCombineComplete(PacketHeader* pStd)
{
	SetVisibleMixItem(0);
...
int TMFieldScene::OnPacketEnvEffect(PacketHeader* pStd)
{
...
int TMFieldScene::OnPacketRESULTGAMBLE(PacketHeader* pStd)
{
	return 0;
}

int TMFieldScene::OnPacketREQArray(PacketHeader* pStd)
{
	return 0;
}

int TMFieldScene::OnPacketAttack(PacketHeader* pStd)
{
	...
	if (pAttacker)
	{
		...
		for (int i = 0; i < 13; ++i)
		{
			if (pAttack->Header.PacketId == MSG_Attack_One_Opcode && i >= 1)
				break;
...
int TMFieldScene::OnPacketNuke(PacketHeader* pStd)
{
	// just that
	return 1;
}
...
int TMFieldScene::OnPacketAutoKick(PacketHeader* pStd)
{
	return 0;
}
...
void TMFieldScene::AirMove_Start(int nIndex)
{
	...
	stAirmoveStart.Header.PacketId = MSG_AirMove_Start_Opcode;
	stAirmoveStart.Header.ClientId = m_pMyHuman->m_dwID;
...
void TMFieldScene::ClearMixPannel()
{
	...
	m_ItemMixClass.m_stCombineItem.Header.ClientId = m_pMyHuman->m_dwID;
	m_ItemMixClass.m_stCombineItem.Header.PacketId = MSG_CombineItemTiny_Opcode;
...
void TMFieldScene::ClearMissionPannel()
{
	...
	m_MissionClass.m_stCombineItem.Header.ClientId = m_pMyHuman->m_dwID;
	m_MissionClass.m_stCombineItem.Header.PacketId = MSG_CombineItemTiny_Opcode;
	m_pGridInvList[0]->m_eGridType = TMEGRIDTYPE::GRID_DEFAULT;
	m_pGridInvList[1]->m_eGridType = TMEGRIDTYPE::GRID_DEFAULT;
	m_pGridInvList[2]->m_eGridType = TMEGRIDTYPE::GRID_DEFAULT;
	m_pGridInvList[3]->m_eGridType = TMEGRIDTYPE::GRID_DEFAULT;
...
void TMFieldScene::InsertInChatList(SListBox* pChatList, STRUCT_MOB *pMobData, SEditableText* pEditChat, unsigned int dwColor, int colorId, unsigned int startId)
{
	...
	sprintf(stMsgWhisper.eCommand, "");
	sprintf(stMsgWhisper.eValue, "%s", pEditChat->GetText());
	BASE_TransCurse(stMsgWhisper.eValue);
	...
	int len = strlen(stMsgWhisper.eValue) + strlen(pMobData->MobName);
	const size_t maxLen = 40;
	if (len <= maxLen)
	{
		char istrText[128]{};
		sprintf(istrText, "[%s]> %s", pMobData->MobName, &stMsgWhisper.eValue[startId]);
	...
	else
	{
		...
		{
			strncpy(dest, stMsgWhisper.eValue, maxLen - 1);
			sprintf(dest2, "%s", &stMsgWhisper.eValue[maxLen - 1]);
		}
		...
		if (strlen(stMsgWhisper.eValue) > maxLen && ipNewItem && pChatList)
			pChatList->AddItem(ipNewItem);
	}
}
