...
void CItemMix::TakeItResource(SControlContainer* ControlContainer, unsigned short CharID)
{
	...
	m_stCombineItem.Header.ClientId = CharID;
}
                    ...
                    if (pItem && pItem->m_GCObj.dwColor == 0xFFFFFFFF)
                    {
                        if (m_dNeedRefer[i].bItemListrefer)
                        {
                                ...
                                && m_dNeedRefer[i].dHavevolume < 1)
                            {
                                ...
                                for (int k = i; k < 8; ++k)
                                {
                                    if (!m_stCombineItem.items[k].sIndex)
                                    {
                                        memcpy(&m_stCombineItem.items[k], pItem->m_pItem, sizeof(m_stCombineItem.items[k]));
                                        m_stCombineItem.slot[k] = 15 * j + 5 * pItem->m_nCellIndexY + pItem->m_nCellIndexX;
                                        break;
                                    }
                                }
                            }
            ...
            if (m_dNeedRefer[i].bItemListrefer)
            {
                    ...
                    !m_pGridNeedItem[i]->m_pItemList[0])
                {
                    ...
                    for (int j = 0; j < 4; ++j)
                    {
                        ...
                        {
                            if (!m_stCombineItem.items[ki].sIndex)
                            {
                                pItem->m_GCObj.dwColor = -65536;
                                memcpy((char*)&m_stCombineItem.items[ki], pItem->m_pItem, sizeof STRUCT_ITEM);

                                m_stCombineItem.slot[ki] = 15 * Sourpage + 5 * pItem->m_nCellIndexY + pItem->m_nCellIndexX;
                                break;
                            }
                        }
...
void CItemMix::DoCombine(SMessagePanel* MessagePanel, SGridControl** GridInvList, int Coin)
{
    ...
    if (count)
    {
        if (count != 1 || m_dNeedRefer[0].dHavevolume >= stNeed_itemList[m_dNeedRefer[0].dindex].dVolume)
        {
            ...
            {
                if (m_stCombineItem.slot[i] < 0 && m_NPCHead != 54)
                {
            ...
            if (stResult_itemList[m_dResultIndex].dCost <= (unsigned int)Coin)
            {
                ...
                {
                    m_stCombineItem.Header.PacketId = stResult_itemList[m_dResultIndex].sMSG;
...
int CItemMix::HardCode(SMessagePanel* MessagePanel, SGridControl** GridInvList)
{
    ...
    {
        auto Item = GridInvList[m_stCombineItem.slot[0] / 15]->GetItem(
            m_stCombineItem.slot[0] % 15 % 5,
            m_stCombineItem.slot[0] % 15 / 5
        );
        
        auto Item2 = GridInvList[m_stCombineItem.slot[1] / 15]->GetItem(
            m_stCombineItem.slot[1] % 15 % 5,
            m_stCombineItem.slot[1] % 15 / 5
        );
        if (Item
    ...
    else if (m_NPCHead == 55)
    {
        for (int i = 0; i < 2; ++i)
        {
            auto Item = GridInvList[m_stCombineItem.slot[i] / 15]->GetItem(
                m_stCombineItem.slot[0] % 15 % 5,
                m_stCombineItem.slot[0] % 15 / 5
            ); // v22
        ...
        auto Item1 = GridInvList[m_stCombineItem.slot[0] / 15]->GetItem(
            m_stCombineItem.slot[0] % 15 % 5,
            m_stCombineItem.slot[0] % 15 / 5
        ); // v22
    ...
    else if (m_NPCHead == 67 && m_dNPCX == 25 && m_dNPCY == 13)
    {
            ...
            || Item->m_pItem->sIndex == 1714)
        {
            ...
            auto Item3 = GridInvList[m_stCombineItem.slot[2] / 15]->GetItem(
                m_stCombineItem.slot[2] % 15 % 5,
                m_stCombineItem.slot[2] % 15 / 5);
