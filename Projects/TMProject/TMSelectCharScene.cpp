...
int TMSelectCharScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	if (idwControlID == 66433)
	{
		if (!g_AccountLock)
		{
			...
			strncpy(msLock.num, keypass, strlen(keypass));
			msLock.num[14] = 0;
			msLock.num[15] = 0;
	...
	if (idwControlID == 4612 && dwServerTime - m_dwLastClickLoginBtnTime > 2000)
	{
		...
		else if (m_pHuman[nSlot] && pSelChar->MobName[nSlot][0])
		{
			...
			stCharacterLogin.Header.ClientId = 0;
			stCharacterLogin.Header.PacketId = MSG_CharacterLogin_Opcode;
	...
	if (idwControlID == 1545)
	{
		...
		if (slotId >= 0 && slotId < 4)
		{
			...
			stNewCharacter.Header.ClientId = 0;
			stNewCharacter.Header.PacketId = MSG_NewCharacter_Opcode;
	...
	if (idwControlID == 628)
	{
		...
		stReqTransper.Header.ClientId = 0;
		stReqTransper.Header.PacketId = MSG_ReqTransper_Opcode;
	...
	if (idwControlID == 65886)
	{
		...
		stDelCharacter.Header.ClientId = 0;
		stDelCharacter.Header.PacketId = MSG_DeleteCharacter_Opcode;
...
int TMSelectCharScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	PacketHeader* pStd = reinterpret_cast<PacketHeader*>(buf);
	...
	{
		g_pTimerManager->SetServerTime(pStd->TimeStamp);
...
int TMSelectCharScene::FrameMove(unsigned int dwServerTime)
{
	...
	if (dwServerTime - LastSendTime > 250000)
	{
		...
		stStandard.ClientId = 0;
		stStandard.PacketId = MSG_Ping_Opcode;

		g_pSocketManager->SendOneMessage((char*)&stStandard, sizeof PacketHeader);
	}
