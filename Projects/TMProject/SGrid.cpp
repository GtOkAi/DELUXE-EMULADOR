		...
		if (pItem)
		{
			if (pItem->m_GCObj.dwColor != 0xFFFF0000)
			{
				...
				if (sDestPos == -1)
				{
					...
					stSwapItem.SrcType = 0;
					stSwapItem.SrcSlot = NewItemPosConv;
					stSwapItem.DstType = sDestType;
					...
					if (sDestType)
					{
						...
						if (page == 3 && g_pObjectManager->m_stMobData.Carry[61].sIndex != 3467)
							return 0;
						stSwapItem.DstSlot = 15 * page + nAX + 5 * nAY;
					}
					else
					{
						stSwapItem.DstSlot = sDestPos;
					}

					if (stSwapItem.DstSlot != stSwapItem.SrcSlot || stSwapItem.SrcType != stSwapItem.DstType)
					{
				...
				Msg.SrcType = 1;
				Msg.SrcSlot = SourPage + vecGrid.x + 5 * vecGrid.y;
				Msg.DstType = sDestType;
				Msg.DstSlot = sDestPos;
				Msg.TargetID = TMFieldScene::m_dwCargoID; 
				
				if ((unsigned char)sDestPos != (unsigned char)Msg.SrcSlot || Msg.SrcType != Msg.DstType)
				{
			...
			if (nItemType == 64 || nItemType == 192)
			{
				if (pMobData->Equip[6].sIndex <= 0)
				{
						...
						&& !bOnlyCheck)
					{
						...
						stSwapItem.Header.PacketId = MSG_SwapItem_Opcode;
						stSwapItem.SrcType = 0;
						stSwapItem.SrcSlot = 6;
						stSwapItem.DstType = static_cast<char>(sDestType);
						stSwapItem.DstSlot = static_cast<char>(sDestPos);
						stSwapItem.TargetID = TMFieldScene::m_dwCargoID;
						SendOneMessage((char*)&stSwapItem, 20);
					}
		...
		if (pItem)
		{
			if (nCellX == 4 && nCellY == 7)
			{
				...
				if (pItem->m_pItem->sIndex == 4998)
				{
					...
					ReqUndoShop.Header.ClientId = pScene->m_pMyHuman->m_dwID;
			...
			MSG_Buy stBuy{};
			stBuy.Header.ClientId = g_pCurrentScene->m_pMyHuman->m_dwID;
			stBuy.Header.PacketId = MSG_Buy_Opcode;
			...
			if (vecGrid.x > -1 && vecGrid.y > -1)
			{
				if (pItem->m_pItem->sIndex != 3331 && pItem->m_pItem->sIndex == 4147)
				{
					...
					pScene->m_stToto.Header.PacketId = 0x3CE;
...
int SGridControl::TradeItem(int nCellX, int nCellY)
{
	...
	if (m_eGridType == TMEGRIDTYPE::GRID_TRADEINV6)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			memcpy(&g_pObjectManager->m_stCombineItem4.items[i], pItem->m_pItem, sizeof(STRUCT_ITEM));
			g_pObjectManager->m_stCombineItem4.slot[i] = SourPos;
			pItem->m_GCObj.dwColor = 0xFFFF0000;
			bEmptyFind = true;
			break;
		}
	...
	if (m_eGridType == TMEGRIDTYPE::GRID_ITEMMIX)
	{
		...
		for (size_t i = 0; i < 13; ++i)
		{
			...
			int Page = g_pObjectManager->m_stCombineItem.slot[i] / 15;
			int CellIndexX = g_pObjectManager->m_stCombineItem.slot[i] % 15 % 5;
			int CellIndexY = g_pObjectManager->m_stCombineItem.slot[i] % 15 / 5;
			...
			memset(&g_pObjectManager->m_stCombineItem.items[i], 0, sizeof(g_pObjectManager->m_stCombineItem.items[i]));
	...
	if (m_eGridType == TMEGRIDTYPE::GRID_ITEMMIX4)
	{
		...
		for (size_t i = 0; i < 3; ++i)
		{
			...
			int Page = g_pObjectManager->m_stCombineItem4.slot[i] / 15;
			int CellIndexX = g_pObjectManager->m_stCombineItem4.slot[i] % 15 % 5;
			int CellIndexY = g_pObjectManager->m_stCombineItem4.slot[i] % 15 / 5;
			...
			memset(&g_pObjectManager->m_stCombineItem4.items[i], 0, sizeof(g_pObjectManager->m_stCombineItem4.items[i]));
			if (g_pCursor->m_pAttachedItem && g_pCursor->m_pAttachedItem == pPickupItem)
				g_pCursor->m_pAttachedItem = nullptr;
...
int SGridControl::SellItem(int nCellX, int nCellY, unsigned int dwFlags, unsigned int wParam)
{
	...
	if (m_eGridType == TMEGRIDTYPE::GRID_SHOP)
	{
		...
		if (g_pEventTranslator->m_bCtrl)
		{
			...
			MSG_Sell stSell{};
			stSell.Header.ClientId = g_pCurrentScene->m_pMyHuman->m_dwID;
...
void SGridControl::SwapItem(int nCellX, int nCellY, int nCellVWidth, int nCellVHeight, STRUCT_ITEM* pItem)
{
		...
		(page != 3 || g_pObjectManager->m_stMobData.Carry[61].sIndex == 3467))
	{
		if (m_eItemType == TMEITEMTYPE::ITEMTYPE_NONE)
		{
			...
			stSwapItem.Header.ClientId = g_pObjectManager->m_dwCharID;
			stSwapItem.Header.PacketId = MSG_SwapItem_Opcode;
			stSwapItem.SrcType = static_cast<char>(CheckType(m_eItemType, m_eGridType));
			stSwapItem.SrcSlot = Sourpage + nCellX + 5 * nCellY;
			stSwapItem.DstType = static_cast<char>(sDestType);
			stSwapItem.DstSlot = Destpage + sDestPos;
			stSwapItem.TargetID = TMFieldScene::m_dwCargoID;

			if (Destpage + sDestPos != stSwapItem.SrcSlot || 
				stSwapItem.SrcType != stSwapItem.DstType)
			{
		...
		else if ((nAtItemPos & (int)m_eItemType) == (int)m_eItemType)
		{
			...
			if (sSrcType || sDestType)
			{
				...
				stSwapItem.Header.ClientId = g_pObjectManager->m_dwCharID;
				stSwapItem.Header.PacketId = MSG_SwapItem_Opcode;
				stSwapItem.SrcType = static_cast<char>(sSrcType);
				stSwapItem.SrcSlot = Sourpage + sSrcPos;
				stSwapItem.DstType = static_cast<char>(sDestType);
				stSwapItem.TargetID = TMFieldScene::m_dwCargoID;

				if (sDestType)
					stSwapItem.DstSlot = Destpage + nAX + 5 * nAY;
				else
					stSwapItem.DstSlot = static_cast<char>(sDestPos);

				if (stSwapItem.DstSlot != stSwapItem.SrcSlot ||
					stSwapItem.SrcType != stSwapItem.DstType)
				{
					SendOneMessage((char*)&stSwapItem, sizeof(stSwapItem));
				}
	...
	else if (pItem->m_pItem->sIndex == 3443)
	{
		if (pItem->m_pItem->stEffect[0].cEffect == 59)
		{
			...
			if (!bFindCapsule)
			{
				...
				dst.Header.PacketId = MSG_RequestCapsuleInfo_Opcode;
				dst.Header.ClientId = g_pCurrentScene->m_pMyHuman->m_dwID;
		...
		pMyHuman->m_eMotion != ECHAR_MOTION::ECMOTION_RUN)
	{
			...
			g_pSpell[cSkillIndex].Passive != 1)
		{
				...
				g_pObjectManager->m_stMobData.CurrentScore.Mp)
			{
				...
				stAttack.Header.ClientId = pMyHuman->m_dwID;
				...
				pFScene->OnPacketEvent(stAttack.Header.PacketId, (char*)&stAttackLocal);
		...
		(m_eGridType == TMEGRIDTYPE::GRID_DEFAULT || m_eGridType == TMEGRIDTYPE::GRID_SELL))
	{
		...
		if (nType == 11 || nType == 13)
		{
			...
			if (g_pCurrentScene->m_eSceneType == ESCENE_TYPE::ESCENE_FIELD)
			{
				...
				pFScene->m_stUseItem.Header.ClientId = g_pObjectManager->m_dwCharID;
				pFScene->m_stUseItem.Header.PacketId = MSG_UseItem_Opcode;
				...
				stDelayStart.Header.ClientId = pMyHuman->m_dwID;
				stDelayStart.Header.PacketId = MSG_DelayStart_Opcode;
				stDelayStart.Parm = 1;
		...
		if (nType == 195)
		{
			...
			pFScene->m_stPotalItem.Header.ClientId = g_pObjectManager->m_dwCharID;
			pFScene->m_stPotalItem.Header.PacketId = MSG_UseItem_Opcode;
			...
			if (pMyHuman)
			{
				if (nItemSIndex == 4030)
				{
					if (!pFScene->m_dwUseItemTime || dwServerTime - pFScene->m_dwUseItemTime >= 200)
					{
						...
						stParam.Header.PacketId = MSG_UseDeclarationOfWar_Opcode;
						stParam.Header.Size = sizeof(stParam);
						stParam.Header.ClientId = g_pCurrentScene->m_pMyHuman->m_dwID;
						stParam.Parm = 0;
...
char SGridControl::automove(int nCellX, int nCellY)
{
	...
	if (pItem && pItem->m_GCObj.dwColor != 0xFFFF0000)
	{
		...
		stSwapItem.Header.ClientId = g_pObjectManager->m_dwCharID;
		stSwapItem.Header.PacketId = MSG_SwapItem_Opcode;
		stSwapItem.SrcType = Type;
		stSwapItem.SrcSlot = page + vecGrid.x + 5 * vecGrid.y;
		stSwapItem.DstType = static_cast<char>(sDestType);
		stSwapItem.DstSlot = static_cast<char>(sDestPos);
		stSwapItem.TargetID = TMFieldScene::m_dwCargoID;
		if ((unsigned char)sDestPos != (unsigned char)stSwapItem.SrcSlot 
			|| stSwapItem.SrcType != stSwapItem.DstType)
		{
			SendOneMessage((char*)&stSwapItem, 20);
