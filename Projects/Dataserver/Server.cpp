...
int ProcessAdminMessage(int conn, char *msg)
{
	...
	case _MSG_NPCreateCharacter:
	{
		...
		memcpy(&file.Mob[empty], &(m->Mob), sizeof(STRUCT_MOB));
...
void ProcessSecTimer()
{
	...
	{/*
		...
		// Enviar pacote para as tmsrv avisando do inacio da guerra
		if (now.tm_hour == 23 && now.tm_wday != 0)
		{
			// Informa os canais das condiaaes iniciais da guerra e libera o funcionamento
			for (int i = 0; i < MAX_SERVERGROUP; i++)
			{
				...
				// Como quem esta avanaadno a o canal contrario
				// Quando o sistema inicia:
				// O canal 1 esta avanaando sobre o 2
				// O canal 2 esta avanaando sobre o 1
				int otherConn = ((i % 2) ? (i - 1) : (i + 1));
				g_pTowerWarState[i].TowerState = otherConn;
			}

			// Envia a informaaao para a TMsrv da atual situaaao da guerra
			// O parametro 1 indica que a guerra esta sendo iniciada
			SendTowerWarInfo(1);
		}
	...
	if (!now.tm_sec && now.tm_wday == 3 && !now.tm_min && now.tm_hour == 19)
	{
		...
		for (int i = 0; i < MAX_SERVERGROUP; i++)
		{
			...
			// E chama a funaao de renascimento de Kefra em cada canal
			if (pUser[i].Mode == USER_EMPTY)
				continue;
	...
	do
	{
		...
		if (pFile != NULL)
		{
			...
			if (num[0])
			{
				...
				str << std::endl << "A senha numarica foi trocada para " << num;
			}
...
{
	char file[256];
	sprintf_s(file, "C:\\xampp\\htdocs\\serv%2.2d.htm", sServer.ServerIndex);
...
void ImportItem()
{
	...
	while (true)
	{
		...
		if (idxName > 0 && idxName < MAX_DBACCOUNT && cFileDB.Account[idxName].Slot >= 0 && cFileDB.Account[idxName].Slot <= 3)
		{
			...
			std::stringstream str;
			str << "Recebido o item atravas da DBsrv " << packet.item.toString().c_str();
		...
		if (!runTimeSent)
		{
			...
			if (pos == -1)
			{
				...
				Log(accountName, "Sem espaao para receber o item %s.", item.toString().c_str());
...
void ImportBan()
{
	...
	while (true)
	{
		...
		memset(&account, 0, sizeof STRUCT_ACCOUNT);
		_strupr_s(accountName);
		memcpy(account.Username, accountName, 16);
...
void ImportCash()
{
	...
	while (true)
	{
		...
		else
		{
			...
			Log(accountName, "Foi entregue um total de %d cash para o usuario. Total: %d", cash, account.Cash + cash);
...
{
	WNDCLASSEX wcex;
	const char* szWindowClass = "W2 - DataBase Server";
	const char* szWindowTitle = "W2 - DataBase Server (v761) Project Deluxe";
	...
	{
		MessageBoxA(NULL, "Nao foi possavel ler o arquivo TRANSKNIGHT", "Error : read", MB_OK);
	...
	{
		MessageBoxA(NULL, "Nao foi possavel ler o arquivo guilds.txt", "Error : read", MB_OK);
	...
	ReadConfig();
	ReadNPCDonate();
	Server[0].ListenServer(GUI.hGUI, 0, 7514, WSA_ACCEPT);

	//AdminSocket.ListenServer(GUI.hGUI, 0, 8895, WSA_ACCEPTADMIN);

	serverLog = std::make_unique<CLog>("..\\Logs\\DBSRV");
