...
CFileDB::CFileDB()
{
	Account = new STRUCT_ACCOUNT_DATABASE[MAX_DBACCOUNT];
	if (!Account)
		exit(1);
	...
	for (INT i = 0; i < MAX_DBACCOUNT; i++)
	{
		...
		memset(&Account[i].Account, 0, sizeof STRUCT_ACCOUNT);
	}
}
...
INT32 CFileDB::DBReadAccount(STRUCT_ACCOUNT *file)
{
	file->Username[15] = 0;
	file->Username[14] = 0;
	file->Password[11] = 0;
...
INT32 CFileDB::DBWriteAccount(STRUCT_ACCOUNT *acc)
{
...
INT32 CFileDB::DBExportAccount(STRUCT_ACCOUNT *file)
{
	STRUCT_ACCOUNT *accName = file;//LOCAL_2
	...
	ret = _write(handle, file, sizeof STRUCT_ACCOUNT_DATABASE);
	if (ret == -1)
	{
		_close(handle);
		return false;
...
void CFileDB::DBGetSelChar(st_CharList *p, STRUCT_ACCOUNT *file)
{
	for (INT32 i = 0; i < 4; i++)
	{
...
void CFileDB::RemoveAccountList(INT32 index)
{
	...
	memset(&Account[index].Account, 0, sizeof STRUCT_ACCOUNT);
}
...
INT32 CFileDB::ProcessMessage(char *msg, INT32 conn)
{
	...
	case _MSG_DBCharacterLogin:
	{
		...
		STRUCT_CHARINFO *mob = &Account[idx].Account.Mob[slot];
		packetExtra.Mob = *mob;

		strncpy_s(packetExtra.Pass, Account[idx].Account.Block.Pass, 16);
		packetExtra.Blocked = Account[idx].Account.Block.Blocked;
			
		memcpy(packet.Affect, Account[idx].Account.Mob[slot].Affects, sizeof STRUCT_AFFECT * 32);
	...
	{//00412EE7
		...
		memcpy(&Account[idx].Account.Ban, &p->Ban, sizeof STRUCT_ITEMDATE);
	...
	case _MSG_DBSaveMob:
	{
		...
		memcpy(&Account[index].Account.Ban, &p->Ban, sizeof STRUCT_ITEMDATE);
	...
	case _MSG_DBNewArch:
	{
		...
		STRUCT_MOB *mob = &Account[index].Account.Mob[slotId].Player;//LOCAL_2844
		STRUCT_CHARINFO *extra = &Account[index].Account.Mob[slotId];
		...
		if (body == 0)
			memcpy(mob, &pBaseSet[0], sizeof STRUCT_MOB);
		else if (body == 1)
			memcpy(mob, &pBaseSet[1], sizeof STRUCT_MOB);
		else if (body == 2)
			memcpy(mob, &pBaseSet[2], sizeof STRUCT_MOB);
		else if (body == 3)
			memcpy(mob, &pBaseSet[3], sizeof STRUCT_MOB);
		else
		{
			SendDBSignal(conn, Header->ClientId, 0x41D);
		...
		for (int i = 1; i < 15; i++)
		{
				memset(&mob->Equip[i], 0, sizeof STRUCT_ITEM);			
		}
	...
	case _MSG_DBNewCharacter:
	{
		...
		STRUCT_MOB *mob = &Account[idx].Account.Mob[slot].Player;//LOCAL_2844
		STRUCT_CHARINFO *extra = &Account[idx].Account.Mob[slot];
	...
	case _MSG_DBAccountLogin:
	{
		...
		STRUCT_ACCOUNT account{};//LOCAL_2126
		...
		if (banType != 0 && account.Ban.Ano != 0 && account.Ban.Mes != 0 && account.Ban.Dia != 0 && master != 0)
		{
			...
			else
			{
				...
				memset(&account.Ban, 0, sizeof STRUCT_ITEMDATE);
			}
		}
		...
		STRUCT_ACCOUNT *pAccount = &Account[idx].Account;//LOCAL_2199
		memcpy(pAccount, &account, sizeof STRUCT_ACCOUNT);
		...
		memcpy(&selChar.Storage[0], &account.Storage.Item[0], sizeof STRUCT_ITEM * 128);
		...
		if(changeServer)
		{
			...
			STRUCT_CHARINFO *mob = &Account[idx].Account.Mob[slot];
			memcpy(&packetExtra.Mob, mob, sizeof STRUCT_CHARINFO);
	...
	case _MSG_DBDeleteCharacter:
	{
		...
		STRUCT_MOB *mob = &Account[index].Account.Mob[slot].Player;//LOCAL_3667
	...
	case MSG_UPDATEWARDECLARATION://0x998
	{
		...
		if (g_pTowerWarState[otherConn].WarState != newInfo && g_pTowerWarState[otherConn].WarState != 2) 
		{
			...
			// Atribui quando esta declarando ou recusando uma guerra
	...
	case MSG_UPDATETOWERINFO://0x999
	{
		...
		// Exemplo
		// Canal 1 derrota canal 1 ira para o else
	...
	*/
	...
	case MSG_PANELGUILD_GETLIST:
		{
			...
			memcpy(&packet.Guild, &g_pGuild, sizeof STRUCT_GUILDINFO * 40);
	...
	case PutInSealPacket:
		...
		const STRUCT_MOB *mob = &Account[index].Account.Mob[slot].Player;//LOCAL_2844
		...
		info.Mob.Escritura = STRUCT_ITEMDATE{};
		...
		info.Mob.PesaEnter = 0;
		info.Mob.Revigorante = STRUCT_ITEMDATE{};
		info.Mob.Saude = STRUCT_ITEMDATE{};
...
void CFileDB::SendAdditionalAccountInfo(INT32 conn, INT32 clientId)
{
	...
	memcpy(&packet.Ban, &Account[idx].Account.Ban, sizeof STRUCT_ITEMDATE);
