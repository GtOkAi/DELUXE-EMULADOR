...
STRUCT_STOREDONATE g_pStore[10][27];
...
BOOL ReadCharBase(INT32 index, const char *fileName)
{
	...
	fread(&pBaseSet[index], sizeof STRUCT_MOB, 1, pFile);
	fclose(pFile);
...
{
	memset(player, 0, sizeof STRUCT_MOB);
